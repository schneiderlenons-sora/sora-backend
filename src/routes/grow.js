...
// Junta no mesmo calendário os compromissos nativos + tudo que já tem data
// nos outros módulos (consultas, contas/receitas fixas, dívidas, faturas,
// manutenções). Cada fonte é tolerante: se a tabela/coluna não existir, o
// feed segue com as demais. Só `compromisso` é editável; o resto é
// read-only e faz deep-link pra área de origem no painel.
function isoLocal(d) {
  const z = new Date(d.getTime() - d.getTimezoneOffset() * 60000);
  return z.toISOString().slice(0, 10);
}
// Datas em que um "dia do mês" (1-31) cai dentro de [de, ate] — pra eventos
...
router.get('/agenda/feed/:phone', auth, requireGrow, async (req, res) => {
  ...
  try {
    const { data } = await supabase.from('compromissos').select('*')
      .eq('grupo_id', grupoId).gte('data', de).lte('data', ate);
  ...
  try {
    ...
    for (const c of data || []) {
      ...
      if (c.retorno_data && c.retorno_data >= de && c.retorno_data <= ate) {
  ...
  try {
    ...
    for (const r of data || []) {
      ...
      for (const d of ocorrenciasMensais(r.dia_vencimento, de, ate)) {
  ...
  try {
    ...
    for (const dv of data || []) {
      ...
      for (const d of ocorrenciasMensais(dv.dia_vencimento, de, ate)) {
  ...
  try {
    ...
    for (const w of data || []) {
      for (const d of ocorrenciasMensais(w.dia_vencimento, de, ate))
      ...
      for (const d of ocorrenciasMensais(w.dia_fechamento, de, ate))
